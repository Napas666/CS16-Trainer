...
# ── Overlay thread (ESP + Crosshair в одном окне) ────────────────────────────
class OverlayThread(threading.Thread):
    def __init__(self, mem, esp: ESPOverlay, xhair: Crosshair):
    ...
    def run(self):
        ...
        win32gui.SetWindowPos(hwnd, win32con.HWND_TOPMOST, rect[0], rect[1], w, h, 0)
        ...
        while not self._stop.is_set():
            ...
            if self.esp.enabled and self.mem.connected:
                self.esp._render(screen, font_sm)

            if self.xhair.enabled:
                self.xhair.draw(screen, w//2, h//2)
...
class TrainerApp(ctk.CTk):
    def __init__(self):
        ...
        self.aim.start()
        self.bhop.start()
        self._overlay = OverlayThread(self.mem, self.esp, self.xhair)
        self._overlay.start()
    ...
    def _build_ui(self):
        ...
        self._status_lbl = ctk.CTkLabel(hdr, text="NOT ATTACHED",
        ...
        tabs = ctk.CTkTabview(self, fg_color=BG_CARD, segmented_button_fg_color=BG_CARD2,
        ...
        self._build_xhair_tab(tabs.tab("✚ ПРИЦЕЛ"))

    # ── AIM tab ───────────────────────────────────────────────────────────────
    def _build_aim_tab(self, tab):
        self._aim_sw = self._sw_row(tab, "AIMBOT", self._toggle_aim)

        self._slider_row(tab, "FOV",    2,  60, 15, 1, self._set_fov,    "°")
        self._slider_row(tab, "SMOOTH", 1,  20,  4, 1, self._set_smooth, "x")

        # Голова / Тело
        row = ctk.CTkFrame(tab, fg_color="transparent")
        row.pack(fill="x", padx=6, pady=(4,2))
        ctk.CTkLabel(row, text="ЦЕЛЬ", font=("Arial",11), text_color="#aaa", width=70).pack(side="left")
        self._aim_target = ctk.CTkSegmentedButton(row, values=["ГОЛОВА","ТЕЛО"],
                                                   command=self._set_aim_target,
                                                   selected_color="#2a1040",
                                                   selected_hover_color="#3a1860",
                                                   unselected_color=BG_CARD2,
                                                   font=("Arial",11,"bold"))
        self._aim_target.set("ГОЛОВА")
    ...
    def _build_esp_tab(self, tab):
        ...
        for col, lbl in [("#ff5050","Прямой вид"), ("#ffc832","За стеной"), ("#50ff78","PENBOX")]:
    ...
    def _build_pen_tab(self, tab):
        ...
        for line in [
            "Зелёный бокс = цель за поверхностью",
            "которую можно простреливать.",
            "",
            "CS 1.6 пробивает металл/дерево",
            "до ~500 ед. в зависимости от оружия.",
            "Бетон / кирпич — непробиваем.",
        ]:
            ctk.CTkLabel(info, text=line, font=("Arial",10),
                         text_color="#666" if line else "#333",
    ...
    def _build_bhop_tab(self, tab):
        ...
        info.pack(fill="x", padx=6, pady=(10,4))
        for line in [
            "Зажми ПРОБЕЛ и держи — bhop",
            "будет работать автоматически.",
            "",
            "Алгоритм: определяет момент",
            "приземления и нажимает SPACE",
            "точно в нужный кадр.",
        ]:
            ctk.CTkLabel(info, text=line, font=("Arial",10),
                         text_color="#777" if line else "#333",
                         justify="left").pack(anchor="w", padx=12, pady=1)

    # ── ПРИЦЕЛ tab ────────────────────────────────────────────────────────────
    def _build_xhair_tab(self, tab):
        self._xh_sw = self._sw_row(tab, "КАСТОМНЫЙ ПРИЦЕЛ", self._toggle_xhair)

        # Стиль
        row = ctk.CTkFrame(tab, fg_color="transparent")
        row.pack(fill="x", padx=6, pady=(6,2))
        ctk.CTkLabel(row, text="СТИЛЬ", font=("Arial",11), text_color="#aaa", width=70).pack(side="left")
        self._xh_style = ctk.CTkOptionMenu(row, values=STYLES,
                                            command=self._set_xh_style,
                                            fg_color=BG_CARD2,
                                            button_color="#2a1040",
                                            button_hover_color="#3a1860",
                                            font=("Arial",11))
        self._xh_style.pack(side="right")

        self._slider_row(tab, "РАЗМЕР",   2, 30, 10, 1, self._set_xh_size,   "px")
        self._slider_row(tab, "ТОЛЩИНА",  1,  6,  2, 1, self._set_xh_thick,  "px")
        self._slider_row(tab, "GAP",      0, 20,  4, 1, self._set_xh_gap,    "px")

        # Точка + Обводка
        self._xh_dot_sw  = self._sw_row(tab, "ЦЕНТРАЛЬНАЯ ТОЧКА", self._toggle_xh_dot)
        self._xh_dot_sw.select()
        self._xh_out_sw  = self._sw_row(tab, "ОБВОДКА", self._toggle_xh_outline)
        self._xh_out_sw.select()

        # Цвет
        row_c = ctk.CTkFrame(tab, fg_color="transparent")
        row_c.pack(fill="x", padx=6, pady=(8,4))
        ctk.CTkLabel(row_c, text="ЦВЕТ", font=("Arial",11), text_color="#aaa", width=70).pack(side="left")
        self._color_preview = ctk.CTkLabel(row_c, text="     ",
                                            fg_color="#00ff50", corner_radius=4, width=32, height=20)
        self._color_preview.pack(side="right", padx=(0,4))
        ctk.CTkButton(row_c, text="Выбрать", command=self._pick_color,
                      fg_color=BG_CARD2, hover_color="#2a1040",
                      font=("Arial",11), height=26, width=80,
    ...
    def _slider_row(self, parent, label, mn, mx, default, step, cmd, unit=""):
        ...
        val_lbl = ctk.CTkLabel(row, text=f"{default}{unit}",
                               font=("Arial",10,"bold"), text_color=ACCENT, width=36)
        val_lbl.pack(side="right")
        def _cb(v):
            val_lbl.configure(text=f"{int(v) if step==1 else round(v,1)}{unit}")
            cmd(v)
    ...
    def _attach(self):
        ...
        if ok:
            ...
            self._status_lbl.configure(text=f"ERROR: {msg[:28]}")
    ...
    def _set_aim_target(self, v):
        self.aim.head_aim = (v == "ГОЛОВА")

    def _set_xh_style(self, v):
        self.xhair.style = STYLES.index(v)

    def _pick_color(self):
        col = colorchooser.askcolor(title="Цвет прицела",
                                    color='#%02x%02x%02x' % self.xhair.color)
        if col and col[0]:
            self.xhair.color = tuple(int(c) for c in col[0])
            hex_c = '#%02x%02x%02x' % self.xhair.color
